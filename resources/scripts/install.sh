...
if ! blkid -s TYPE -o value "$SWAP_DEVICE" | grep -qx swap; then
    mkswap "$SWAP_DEVICE"
fi
if ! swapon --show=NAME | grep -qx "$(readlink -f "$SWAP_DEVICE")"; then
    swapon "$SWAP_DEVICE"
fi
SWAP_UUID=$(blkid -s UUID -o value "$SWAP_DEVICE")
grep -q "^UUID=$SWAP_UUID " /etc/fstab || echo "UUID=$SWAP_UUID none swap sw,nofail 0 0" >> /etc/fstab
...
mountpoint -q "$GITLAB_HOME" || mount "$GITLAB_DEVICE" "$GITLAB_HOME"
FS_UUID=$(blkid -s UUID -o value "$GITLAB_DEVICE")

grep -q "^UUID=$FS_UUID " /etc/fstab || echo "UUID=$FS_UUID $GITLAB_HOME xfs defaults,nofail 0 2" >> /etc/fstab
